...
if (!ACROPOLIS_URL || !DBSYNC_URL) {
  throw new Error(
    "Missing required environment variables ACROPOLIS_REST_URL or MAINNET_DBSYNC_URL",
  );
}
...
async function queryAcropolis(epoch: number) {
  const { data, status } = await axios.get(
    `${ACROPOLIS_URL}/spdd?epoch=${epoch}`,
    {
      validateStatus: () => true,
    },
  );
  ...
  for (const [pool_id, info] of Object.entries(data)) {
    ...
    // Use 'live' stake (utxo + rewards) to match db-sync epoch_stake.amount
    ...
    try {
      ...
      if (err.message.startsWith("HTTP")) {
        if (epoch == START_EPOCH) {
          console.log(
            `store-spdd=false or sync has not reached epoch ${START_EPOCH}.`,
          );
        } else {
