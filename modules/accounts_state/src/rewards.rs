...
use acropolis_common::{
    rational_number::RationalNumber, KeyHash, Lovelace, RewardAccount, ShelleyParams,
};
    ...
    pub fn calculate_rewards(
        ...
        for (operator_id, spo) in self.go.spos.iter() {
            ...
            let pool_owner_stake =
                self.go.get_stake_delegated_to_spo_by_addresses(&operator_id, &spo.pool_owners);
            ...
            let capped_relative_pool_stake =
                min(&relative_pool_stake, &relative_pool_saturation_size);
            ...
            let optimum_rewards = ((&stake_rewards
                / (BigDecimal::one() + &pledge_influence_factor))
