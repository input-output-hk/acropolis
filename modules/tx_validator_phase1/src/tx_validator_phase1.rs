...
impl TxValidatorPhase1 {
    ...
    pub async fn init(&self, context: Arc<Context<Message>>, config: Arc<Config>) -> Result<()> {
        ...
        let publish_validation_result = config.get_string("publish-validation-result-phase1").ok();
        if let Some(ref topic) = publish_validation_result {
        ...
        context.clone().run(async move {
            State::run().await.unwrap_or_else(|e| error!("TX validator failed: {e}"));
        });
