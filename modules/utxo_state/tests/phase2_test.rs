...
const UNISWAP: &[u8] = include_bytes!("../../../tests/fixtures/plutus_scripts/uniswap-3.flat");
const VESTING: &[u8] = include_bytes!("../../../tests/fixtures/plutus_scripts/vesting-1.flat");
...
fn all_fixture_scripts_evaluate_successfully() {
    ...
    for (name, bytes) in scripts {
        ...
        let elapsed_ms = result.as_ref().map(|d| d.as_secs_f64() * 1000.0).unwrap_or(0.0);
        println!(
            "  {name:<30} {elapsed_ms:>8.3}ms  {}",
...
fn fixture_scripts_within_performance_threshold() {
    ...
    for (name, bytes) in scripts {
        ...
        let mut timings: Vec<f64> = (0..5)
            .map(|_| evaluate_raw_flat_program(bytes).unwrap().as_secs_f64() * 1000.0)
            .collect();
