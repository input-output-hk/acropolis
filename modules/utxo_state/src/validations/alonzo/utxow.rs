                ...
                utxo.address.get_payment_part()
            {
                if let Some(script_lang) = scripts_provided.get(&script_hash) {
                    if script_lang.is_native() {
                        // For native scripts, we don't need to check datum
                        continue;
                    }
...
pub fn validate_datums(
    ...
    for datum_hash in input_datum_hashes.iter() {
        if !plutus_data.iter().any(|(hash, _)| *hash == *datum_hash) {
    ...
    let tx_datum_hashes = plutus_data.iter().map(|(hash, _)| *hash).collect::<HashSet<_>>();
...
pub fn validate_redeemers(
    ...
    let redeemers_needed = scripts_needed
        ...
        .filter(|(_, script_hash)| {
            scripts_provided
                .get(*script_hash)
                .map(|script_type| !script_type.is_native()) // Only Plutus scripts need redeemers
                .unwrap_or(false)
        })
