...
use acropolis_common::{
    messages::EnactStateMessage, 
    Committee, CommitteeChange, ConwayParams, Era, EnactStateElem, ProtocolParams, ProtocolParamUpdate
};
...
impl ParametersUpdater {
    pub fn new() -> Self {
        Self { params: ProtocolParams::default() }
    }
    ...
    fn update_committee(c: &mut Committee, cu: &CommitteeChange) {
        for removed_member in cu.removed_committee_members.iter() {
            if let None = c.members.remove(removed_member) {
                error!("Removing {:?}, which is not a part of the committee", removed_member);
            }
        }
        for (new_member, v) in cu.new_committee_members.iter() {
            if let Some(old) = c.members.insert(new_member.clone(), *v) {
                error!("New committee member {:?} replaces the old committee member {:?}", (new_member,v), old);
            }
        }
