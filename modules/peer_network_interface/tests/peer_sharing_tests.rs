...
fn loopback_ipv4_rejected() {
    assert!(validate_and_normalise("127.0.0.1", 3001).is_none());
}

#[test]
fn loopback_ipv6_rejected() {
    assert!(validate_and_normalise("::1", 3001).is_none());
}

#[test]
fn private_10_rejected() {
    assert!(validate_and_normalise("10.0.0.1", 3001).is_none());
}

#[test]
fn private_172_rejected() {
    assert!(validate_and_normalise("172.16.0.1", 3001).is_none());
}

#[test]
fn private_192_rejected() {
    assert!(validate_and_normalise("192.168.1.1", 3001).is_none());
}

#[test]
fn link_local_ipv4_rejected() {
    assert!(validate_and_normalise("169.254.1.1", 3001).is_none());
}
...
fn ipv4_mapped_ipv6_normalised_to_ipv4() {
    ...
    let result = validate_and_normalise(&mapped.to_string(), 3001);
    assert_eq!(result, Some("1.2.3.4:3001".to_string()));
}

#[test]
fn public_ipv4_accepted() {
    assert!(validate_and_normalise("185.40.4.100", 3001).is_some());
    assert_eq!(
        validate_and_normalise("185.40.4.100", 3001),
...
fn unspecified_ipv4_rejected() {
    assert!(validate_and_normalise("0.0.0.0", 3001).is_none());
}

#[test]
fn unspecified_ipv6_rejected() {
    assert!(validate_and_normalise("::", 3001).is_none());
}
...
/// Spawn a mock server that immediately closes the connection (simulates a server that
/// doesn't speak pallas protocol at all, causing a connection/handshake error).
async fn spawn_connection_drop_server() -> SocketAddr {
...
async fn timeout_is_respected_when_server_hangs() {
    ...
    // 1-second timeout
    let result = request_peers(&addr.to_string(), 764824073, 15, Duration::from_secs(1)).await;
