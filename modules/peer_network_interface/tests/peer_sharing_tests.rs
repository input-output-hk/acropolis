...
fn loopback_ipv4_accepted() {
    assert_eq!(
        validate_and_normalise("127.0.0.1", 3001, true),
...
fn loopback_ipv6_accepted() {
    assert_eq!(
        validate_and_normalise("::1", 3001, true),
...
fn private_10_accepted() {
    assert_eq!(
        validate_and_normalise("10.0.0.1", 3001, true),
...
fn private_172_accepted() {
    assert_eq!(
        validate_and_normalise("172.16.0.1", 3001, true),
...
fn private_192_accepted() {
    assert_eq!(
        validate_and_normalise("192.168.1.1", 3001, true),
...
fn link_local_ipv4_rejected() {
    assert!(validate_and_normalise("169.254.1.1", 3001, true).is_none());
}

#[test]
fn link_local_ipv6_rejected() {
    assert!(validate_and_normalise("fe80::1", 3001, true).is_none());
}

#[test]
fn port_zero_rejected() {
    assert!(validate_and_normalise("1.2.3.4", 0, true).is_none());
}
...
fn ipv4_mapped_ipv6_normalised_to_ipv4() {
    ...
    let result = validate_and_normalise(&mapped.to_string(), 3001, true);
    assert_eq!(result, Some("1.2.3.4:3001".to_string()));
}

#[test]
fn public_ipv4_accepted() {
    assert!(validate_and_normalise("185.40.4.100", 3001, true).is_some());
    assert_eq!(
        validate_and_normalise("185.40.4.100", 3001, true),
...
fn unspecified_ipv4_rejected() {
    assert!(validate_and_normalise("0.0.0.0", 3001, true).is_none());
}

#[test]
fn unspecified_ipv6_rejected() {
    assert!(validate_and_normalise("::", 3001, true).is_none());
}

#[test]
fn ipv6_rejected_when_disabled() {
    assert!(
        validate_and_normalise("2a05:d014:1fd:cb00:d9a0:4b62:beef:cafe", 3001, false).is_none()
    );
}
...
fn ipv4_mapped_ipv6_accepted_when_ipv6_disabled() {
    ...
    let mapped = Ipv6Addr::from([0, 0, 0, 0, 0, 0xffff, 0x0102, 0x0304]);
    let result = validate_and_normalise(&mapped.to_string(), 3001, false);
    assert_eq!(result, Some("1.2.3.4:3001".to_string()));
}
...
fn public_ipv6_accepted_when_enabled() {
    assert_eq!(
        validate_and_normalise("2a05:d014:1fd:cb00:d9a0:4b62:beef:cafe", 3001, true),
