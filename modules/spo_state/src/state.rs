...
impl State {
    ...
    pub fn handle_spdd(&mut self, block: &BlockInfo, spdd_message: &SPOStakeDistributionMessage) {
        ...
        spos.par_iter().for_each(|(key, value)| {
            {
                // update active stakes
                let mut active_stakes =
                    self.active_stakes.entry(key.clone()).or_insert(VecDeque::new());
            ...
            }
            {
                ...
                if let Some(epochs_history) = &self.epochs_history {
                    let mut current = epochs_history.entry(key.clone()).or_insert(BTreeMap::new());
                    current
                        .entry(block.epoch + 1)
    ...
    pub fn handle_epoch_activity(
        ...
        vrf_vkey_hashes.iter().for_each(|(vrf_vkey_hash, amount)| {
            *(total_blocks_minted.entry(vrf_vkey_hash.clone()).or_insert(0)) += *amount as u64;
            ...
            // update epochs history if enabled
            if let Some(epochs_history) = &self.epochs_history {
                // since we have vrf_key_hash, and key is pool_operator.
                // need to think about it.
            }
        });

        let new_total_blocks_minted_state = TotalBlocksMintedState {
            block: block.number,
            epoch: block.epoch,
            total_blocks_minted,
        };
        ...
        self.total_blocks_minted_history.push_back(new_total_blocks_minted_state);
    }
...
pub mod tests {
    ...
    #[tokio::test]
    async fn get_pools_active_stakes_returns_zeros_when_state_is_new() {
        let state = State::new(false);
        assert_eq!(
            (vec![0, 0], 0),
            state.get_pools_active_stakes(&vec![vec![1], vec![2]], 0)
        );
    }

    #[tokio::test]
    async fn get_pools_active_stakes_returns_zeros_when_not_found() {
        let mut state = State::new(false);
    ...
    #[tokio::test]
    async fn front_active_stakes_removed() {
        let mut state = State::new(false);
        let mut msg = new_spdd_message();
        msg.spos = vec![
            (
        ...
        block.epoch = 2;
        state.handle_spdd(&block, &msg);
        assert_eq!(1, state.active_stakes.get(&vec![1]).unwrap().len());

        msg.epoch = 2;
        block.number = 21;
        block.epoch = 3;
        state.handle_spdd(&block, &msg);
        assert_eq!(2, state.active_stakes.get(&vec![1]).unwrap().len());

        msg.epoch = 3;
        block.number = 31;
        block.epoch = 4;
        state.handle_spdd(&block, &msg);
        // one is popped from the front
        assert_eq!(2, state.active_stakes.get(&vec![1]).unwrap().len());
        assert_eq!(
            (vec![10, 20], 30),
            state.get_pools_active_stakes(&vec![vec![1], vec![2]], block.epoch + 1)
