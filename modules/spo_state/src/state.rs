...
struct StableState {
    ...
    stake_addresses:
        Option<BTreeMap<StakeAddress, acropolis_common::stake_addresses::StakeAddressState>>,
}
...
impl serde::Serialize for State {
    ...
    where
        S: serde::Serializer,
    {
        let stake_addresses = self.stake_addresses.as_ref().map(|stake_addresses| {
            let stake_addresses = stake_addresses.lock().unwrap();
            stake_addresses
                .iter()
                .map(|(stake_address, stake_state)| (stake_address.clone(), stake_state.clone()))
                .collect()
        });
...
impl State {
    ...
    pub fn rollback_debug_summary(&self) -> String {
        ...
        let stake_addresses_summary = self.stake_addresses.as_ref().map(|stake_addresses| {
            let stake_addresses = stake_addresses.lock().unwrap();
            let stable_stake_addresses: BTreeMap<
                StakeAddress,
                acropolis_common::stake_addresses::StakeAddressState,
            > = stake_addresses
                .iter()
                .map(|(stake_address, stake_state)| (stake_address.clone(), stake_state.clone()))
                .collect();
            format!(
                "len={} {}",
                stake_addresses.len(),
                debug_fingerprint(&stable_stake_addresses)
            )
        });
