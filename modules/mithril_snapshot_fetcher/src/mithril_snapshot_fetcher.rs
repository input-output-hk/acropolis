...
impl RollbackReplayBoundary {
    ...
    fn rollback_marker_block_info(&self) -> BlockInfo {
        self.rollback_to.as_ref().unwrap_or(&self.replay_from).with_status(BlockStatus::RolledBack)
    }
...
mod tests {
    ...
    fn rollback_replay_boundary_uses_previous_block_as_rollback_point() {
        ...
        assert_eq!(
            boundary.rollback_marker_block_info().number,
            previous.number
        );
        assert_eq!(boundary.rollback_marker_block_info().slot, previous.slot);
        assert_eq!(
    ...
    #[test]
    fn rollback_replay_boundary_falls_back_to_replay_from_when_previous_missing() {
        let replay_from = test_block(100, 1000, 5, true, true);
        ...
        assert_eq!(boundary.rollback_marker_block_info().slot, replay_from.slot);
        assert_eq!(
            boundary.rollback_marker_block_info().status,
            BlockStatus::RolledBack
        );
