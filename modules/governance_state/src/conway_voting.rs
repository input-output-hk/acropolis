...
impl ActionStatus {
    ...
    /// Meant to be used to delay expiration for one epoch, if previous action ratification changes
    /// Committee. Must be called when the action is active.
    /// TODO: check whether this is really needed. Haskell implementation seems not making these
    /// delays (that's why function is not used now), however some docs mention such behaviour.
    #[allow(dead_code)]
    pub fn delay_expiration(&mut self, curr_epoch: u64) -> Result<()> {
        anyhow::ensure!(
            self.is_active(curr_epoch)
                && self.ratification_epoch.is_none()
                && self.enactment_epoch.is_none()
                && self.expiration_epoch.is_none()
        );
        self.voting_epochs.end += 1;
        Ok(())
    }
...
impl ConwayVoting {
    ...
    fn get_actual_votes(
        ...
        if !self.is_bootstrap()? {
            ...
            for (pool, stake) in spo_stake.iter() {
                let vote = match &stake.default_vote {
